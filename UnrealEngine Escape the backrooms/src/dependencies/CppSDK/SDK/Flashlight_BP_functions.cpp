#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: Flashlight_BP

#include "Basic.hpp"

#include "Flashlight_BP_classes.hpp"
#include "Flashlight_BP_parameters.hpp"


namespace SDK
{

// Function Flashlight_BP.Flashlight_BP_C.ExecuteUbergraph_Flashlight_BP
// (Final, UbergraphFunction, HasDefaults)
// Parameters:
// int32                                   EntryPoint                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void AFlashlight_BP_C::ExecuteUbergraph_Flashlight_BP(int32 EntryPoint)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Flashlight_BP_C", "ExecuteUbergraph_Flashlight_BP");

	Params::Flashlight_BP_C_ExecuteUbergraph_Flashlight_BP Parms{};

	Parms.EntryPoint = EntryPoint;

	UObject::ProcessEvent(Func, &Parms);
}


// Function Flashlight_BP.Flashlight_BP_C.ReceiveBeginPlay
// (Event, Protected, BlueprintEvent)

void AFlashlight_BP_C::ReceiveBeginPlay()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Flashlight_BP_C", "ReceiveBeginPlay");

	UObject::ProcessEvent(Func, nullptr);
}

}

